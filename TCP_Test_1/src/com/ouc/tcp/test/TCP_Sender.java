...
//        System.out.println("SR Sender: Sent Seq=" + nextSeqNum);
    ...
    public void waitACK() {
        while (!ackQueue.isEmpty()) {
            ...
            // SR:收到哪个 ACK 就停哪个定时器并标记
            if (sndpkt.containsKey(currentAck) && !acked.get(currentAck)) {
                // 停止并移除该包的定时器
    ...
    // SR独立定时器管理
    private void start_timer(final int seq) {
        ...
        reTrans = new UDT_RetransTask(client, null) {
            ...
            public void run() {
                // 已经确认了，直接退出不重传
                if (!acked.containsKey(seq) || acked.get(seq)) {
                    this.cancel();
                    return;
                }
                TCP_PACKET p = sndpkt.get(seq);
                if (p != null) {
                    client.send(p); // 仅重传这一个包
                    start_timer(seq); // 重启该包计时
                }
            }
        };
